...
def _esc(text: str) -> str:
    """转义 textual.markup 的特殊字符 \\、[、]。

    textual 用自己的 textual.markup 解析器（不是 rich.markup）：\\ 转义 [，但
    标签内容里裸露的 ] 会被当作标签结束符。所以 [bold]\\[1][/] 里 \\[1] 的 ]
    会提前闭合 [bold]，使随后的 [/] 变成无配对的 auto-close → MarkupError。
    rich.markup.escape 也不够（它不转义 [1] 这类数字标签）。因此这里手动把
    \\、[、] 全部转义，保证任意用户文本在 markup 里都是字面量。
    """
...
class InlineAskUserWidget(Vertical, can_focus=True):
    """
        ...
        带 ☐/☑ 勾选标记的导航栏、光标导航、
        多选（MultiSelect）切换、"Other" 自定义输入，复核/提交视图。
    """
    ...
    def _render_question(self) -> str:
        ...
        # 动态文本一律 escape，避免问题/选项里的 [ ] 被 Rich 当成 markup 标签
        header = _esc(str(q.get("question", q.get("message", f"Question {self._q_idx + 1}"))))
        lines.append(f" [bold #875FFF]{header}[/]\n")
        ...
        for i, opt in enumerate(options):
            label = _esc(opt.get("label", str(opt)) if isinstance(opt, dict) else str(opt))
            ...
            desc_part = f" — [dim]{_esc(str(desc))}[/]" if desc else ""
        ...
        if cursor == other_idx:
            ...
            # 用户键入的文本转义；空时用字面占位符（其 markup 是有意的）
            display = _esc(text) if text else "[dim]Type your answer here...[/]"
    ...
    def _render_nav_bar(self) -> str:
        ...
        for i, q in enumerate(self._questions):
            header = _esc(str(q.get("header", f"Q{i+1}")))
    ...
    def _render_submit(self) -> str:
        ...
        for i, q in enumerate(self._questions):
            header = _esc(str(q.get("header", q.get("question", f"Q{i+1}"))))
            ans = self._answered.get(i, "")
            if ans:
                lines.append(f"   {header}: {_esc(str(ans))}")
            else:
