"""
MCPManager mcp管理类， 管理多个mcp client 及其 Tool
"""
...
from aicocode.config import MCPServerConfig
from aicocode.mcp.client import MCPClient
from aicocode.mcp.tool_wrapper import MCPTool
...
class MCPManager:
    ...
    async def connect_all(self) -> ConnectResult:
        """
            ...
            连接后从 InitializeResult 提取 instructions，将其包含在 ServerInfo 中返回，供系统提示注入使用。
        """
        ...
        for name, config in self._configs.items():
            try:
                ...
                for tool_def in tools:
                    mcp_tool = MCPTool(name, tool_def, client)
                    result.tools.append(mcp_tool)
                    logger.info("Registered MCP tool: %s", mcp_tool.name)

            except Exception as e:
                msg = f"MCP server '{name}' connect error: {e}"
                logger.warning(msg)
    ...
    async def register_all_tools(self, registry: ToolRegistry) -> ConnectResult:
        """
            ...
            返回 ConnectResult，可通过 result.servers获取每个服务器的 instructions。
        """
