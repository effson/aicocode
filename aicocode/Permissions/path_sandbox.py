...
class PathSandbox:
    ...
    _DEFAULT_DENY_WRITE_PATH: list[str] = [
        ".aicocode/config.yaml",
        ".aicocode/permissions.local.yaml",
        ".aicocode/skills/",
    ]
    ...
    def __init__(
        ...
        for dp in (deny_write or self._DEFAULT_DENY_WRITE_PATH):
            dp_path = Path(dp)
    ...
    def _is_deny_write(self, real_path: Path) -> bool:
        """
            检查路径是否命中禁写列表。支持目录前缀匹配：如果禁写项以 / 结尾或本身是目录，
            则该目录下的所有文件都被禁止写入。
        """
