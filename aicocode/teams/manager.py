...
from aicocode.teams.backend_detect import BackendDetectionError, detect_backend
from aicocode.teams.mailbox import Mailbox, create_message
from aicocode.teams.models import (
    AgentTeam,
    BackendType,
    TeammateInfo,
    resolve_team_dir,
...
from aicocode.teams.progress import TeammateProgress
from aicocode.teams.registry import AgentNameRegistry
from aicocode.teams.shared_task import SharedTaskStore
from aicocode.teams.spawn_inprocess import InProcessTeammateHandle
...
class TeamManager:
    ...
    def create_team(
        ...
        team_dir = resolve_team_dir(slug)   # /.aicocode/teams/slug/
        team_dir.mkdir(parents=True, exist_ok=True)

        config_path = str(team_dir / "config.json") # /.aicocode/teams/slug/config.json
        team = AgentTeam(
            name=slug,
        ...
        task_store = SharedTaskStore(team_dir / "tasks.json") # /.aicocode/teams/slug/tasks.json
        task_store.init_empty()

        mailbox_dir = team_dir / "inboxes"  # /.aicocode/teams/slug/inboxes
    ...
    def _kill_pane(self, pane_id: str, backend_type: str) -> None:
        try:
            if backend_type == BackendType.TMUX.value:
                from aicocode.teams.spawn_tmux import kill_pane
                kill_pane(pane_id)
            elif backend_type == BackendType.ITERM2.value:
                from aicocode.teams.spawn_iterm2 import kill_pane
                kill_pane(pane_id)
        except Exception as e:
