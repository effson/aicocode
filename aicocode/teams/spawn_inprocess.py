...
from aicocode.teams import protocol
from aicocode.teams.mailbox import Mailbox, MailboxMessage, create_message
from aicocode.teams.progress import TeammateProgress, random_verb

if TYPE_CHECKING:
    from aicocode.agent import Agent
    from aicocode.conversation import Conversation
    from aicocode.teams.models import TeammateInfo
...
def _plan_mode_active(agent: Any) -> bool:
    ...
    return bool(getattr(agent, "in_plan_mode", False))
        ...
        for m in mailbox.consume(member_name):
            ...
            if (m.type == protocol.PLAN_APPROVAL_RESPONSE
                ...
                if protocol.approved(m):
                    # 批准后切回正常权限，队友可以改文件了
                    from aicocode.Permissions.permission_mode import PermissionMode
...
def spawn_inprocess_teammate(
    ...
    async def _run() -> str:
        """
        teammate 主循环。有 mailbox 时进入长驻循环：执行 agent → 发 idle 通知 →
        轮询等待新任务。没有 mailbox 时退化为单次执行（向后兼容）。
        """
        try:
            if conversation is not None:
                conv = conversation
            else:
                from aicocode.conversation import Conversation as CM
                conv = CM()
            ...
            while True:
                ...
                # 计划模式的teammate：一轮跑完意味着它调了 ExitPlanMode，计划已经落到磁盘。
