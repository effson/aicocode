...
# 获取 项目级 memory.md文件路径
def get_auto_mem_path(project_root: str) -> str:
    """
    返回项目级记忆目录路径：<projectRoot>/.aicocode/memory/。
    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 AICOCODE_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("AICOCODE_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".aicocode", "memory") + os.sep

# 获取 用户级 memory.md文件路径
def get_user_auto_mem_path() -> str:
    """
    返回用户级记忆目录路径：~/.aicocode/memory/。
    ...
    if not home:
        return ""
    return os.path.join(home, ".aicocode", "memory") + os.sep
...
def build_memory_prompt(user_mem_dir: str, project_mem_dir: str) -> str:
    ...
    组合类型化记忆行为指令 + 两个 MEMORY.md的内容，生成完整的 '# auto memory' 系统提示段。
    """
...
class MemoryManager:
    ...
    def __init__(self, project_root: str) -> None:
        ...
        # 用户级：~/.aicocode/memory/ — user/feedback 类型记忆
        self._user_mem_dir = get_user_auto_mem_path()
        # 项目级：<projectRoot>/.aicocode/memory/ — project/reference 类型记忆
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".aicocode" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.aicocode/memory/）。"""
        return Path(self._user_mem_dir.rstrip(os.sep)) if self._user_mem_dir else Path.home() / ".aicocode" / "memory"

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.aicocode/memory/）。"""
    ...
    async def extract(
        ...
        recent = conversation.messages[self._last_extraction_msg_count:]
        if not recent:
            return
        ...
        extract_conv.messages = [Message(role="user", content=prompt)]
        ...
        self._last_extraction_msg_count = len(conversation.messages)
