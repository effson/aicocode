...
def load_instructions(project_root: str) -> str:
    """
    发现并拼接项目和用户指令文件。发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.aicocode/AICOCODE.md, ~/.aicocode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 AICOCODE.md 和 AGENTS.md
    3. workDir/.aicocode/INSTRUCTIONS.md（遗留格式）
    4. workDir/AICOCODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".aicocode" / "AICOCODE.md")
    _add(home / ".aicocode" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".aicocode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "AICOCODE.local.md")
