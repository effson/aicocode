...
from aicocode.memory.auto_memory import ENTRYPOINT_NAME, MAX_ENTRYPOINT_LINES
from aicocode.memory.session import SessionManager
...
class MemoryAutoDreamer:
    def __init__(
        self,
        work_dir: str,
        *,
        min_hours: int = DEFAULT_MIN_HOURS, # 24
        min_sessions: int = DEFAULT_MIN_SESSIONS, # 5
    ) -> None:
        self._work_dir = work_dir
        self._mem_dir = os.path.join(work_dir, ".aicocode", "memory")
        self._user_mem_dir = os.path.join(Path.home(), ".aicocode", "memory")
        self._min_hours = min_hours
        self._min_sessions = min_sessions  # 5
    ...
    async def maybe_run(
        ...
        """检查门控条件，满足则后台执行一次整理。"""
        logger.info("[auto-dream] maybe_run start")
        if not os.path.isdir(self._mem_dir):
            return

        # 时间
        last_at = _read_last_auto_dream_at(self._mem_dir)
        ...
        if len(session_ids) < self._min_sessions: # 5
            return
        ...
        logger.debug(
            "[auto-dream] firing — %.1fh since last, %d sessions",
            hours_since,
            len(session_ids),
        )
    ...
    async def _run(
        ...
        try:
            await self._do_auto_dream(
                client, conversation, protocol, session_ids
            )
        except Exception:
            logger.debug("[auto_dream] failed, rolling back lock")
            _rollback_lock(self._mem_dir, prior_mtime)

    async def _do_auto_dream(
        self,
        client: Any,
        conversation: Any,
        protocol: str,
        ...
        from aicocode.Permissions.validator import PermissionValidator
        ...
        transcript_dir = os.path.join(self._work_dir, ".aicocode", "sessions")
        prompt = _build_auto_dream_prompt(
        ...
        checker = PermissionValidator(self._work_dir, permission_mode="bypass")
        ...
        logger.debug("[auto_dream] completed")
...
def _read_last_auto_dream_at(mem_dir: str) -> int:
