...
from aicocode.agent_event import PermissionResponse
...
class InlinePermissionWidget(Vertical, can_focus=True):
    ...
    def _build_content(self) -> str:
        ...
        for i, (label, _resp) in enumerate(_PERMISSION_OPTIONS):
            if i == self._cursor:
    ...
    def action_cursor_down(self) -> None:
        if self._cursor < len(_PERMISSION_OPTIONS) - 1:
            self._cursor += 1
            self._refresh()

    def action_select(self) -> None:
        _, response = _PERMISSION_OPTIONS[self._cursor]
