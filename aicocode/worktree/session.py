...
from aicocode.worktree.models import WorktreeSession

log = logging.getLogger(__name__)

SESSION_FILENAME = "worktree_session.json"

def _session_path(aicocode_dir: Path) -> Path:
    return aicocode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(aicocode_dir)
    if session is None:
...
def load_worktree_session(aicocode_dir: Path) -> WorktreeSession | None:
    path = _session_path(aicocode_dir)
    if not path.exists():
        return None
    try:
