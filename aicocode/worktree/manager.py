...
from aicocode.worktree.changes import (
    CleanupResult,
    Changes,
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".aicocode" / "worktrees"
        )
        self._aicocode_dir = Path(repo_root) / ".aicocode"
    ...
    def read_worktree_head_sha(wt_path: str) -> str | None:
        """
        不启动 git子进程的情况下，直接读取磁盘上的.git文件和元数据目录，快速获取 Worktree（工作树）
        当前 HEAD对应的CommitSHA（40位哈希值）
        """
        wt = Path(wt_path)
        git_file = wt / ".git"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._aicocode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._aicocode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._aicocode_dir)
        if session is None:
            return None
