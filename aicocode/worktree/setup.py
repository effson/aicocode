"""
Git Worktree 创建后自动初始化环境的核心处理模块，主要解决
-环境缺失、配置丢失、软链接缺失和 Git 校验失效等
"""
...
def perform_post_creation_setup(
    ...
    _copy_local_configs(root, wt)   # 复制本地配置文件
    _setup_git_hooks(root, wt)  # 共享并继承 Git 钩子
    _create_symlinks(root, wt, symlink_directories or [])  # 挂载指定的共享目录（软链接）
...
def _copy_ignored_files(root: Path, wt: Path) -> None:
    """
    根据主仓库下的自定义配置文件 .worktreeinclude 中的规则，从主仓库被 .gitignore
    忽略的文件列表中，把符合条件的文件拷贝到新 Worktree
    """
