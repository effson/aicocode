...
from aicocode.message_adaptor import(
    construct_anthropic_messages,
    construct_openai_input,
    construct_chat_completion_messages,
    construct_messages,
)

DEFAULT_CONFIG = "./.aicocode/config.yaml"
...
class AnthropicClient(LLMClient):
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .aicocode/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    def _mark_last_user_tail_for_cache(self, messages: list[dict[str, Any]]) -> None:
        """
            给最后一条 user 消息的最后一个 block 附加 cache_control。
            仅适用于 Anthropic 协议 message。
        """
        if not messages:
            return
    ...
    def _mark_last_tool_for_cache(self, tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
            返回浅拷贝的 tools 列表，最后一个 tool 标记 cache_control。
        """
        if not tools:
            return tools
        marked = list(tools)
    ...
    async def stream(
            ...
            messages = construct_anthropic_messages(conversation.fetch_messages())
            ...
            if tools:
                kwargs["tools"] = self._mark_last_tool_for_cache(tools)
            ...
            """            
            MiniMax 等 Anthropic 兼容 provider 会在 message_start 里返回input_tokens=0，而把真实的 
            input_tokens/cache_read_input_tokens/cache_creation_input_tokens 放在 message_delta 的 usage 里。
            标准Anthropic SDK 的 MessageDeltaUsage 只有 output_tokens，因此需要用getattr 安全读取这些非标准字段并暂存，
            在最终构造 StreamEnd 时作为降级值使用。
            """
...
class OpenAIClient(LLMClient):
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .aicocode/config.yaml or via OPENAI_API_KEY env var."
            )
    ...
    async def stream(
        ...
        input_messages = construct_openai_input(conversation.fetch_messages())
...
class OpenAICompatibleClient(LLMClient):
    ...
    def _convert_tools(tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        把 tool schema 转换成 Chat Completions 格式。tool 注册表为 ``openai`` 系列输出的是 Responses API 风格的 dict::
        {"type": "function", "name": "...", "description": "...", "parameters": {...}}

        而 Chat Completions 要求把 name/description/parameters 嵌套在``function`` 键下::
    ...
    async def stream(
        ...
        messages = construct_chat_completion_messages(conversation.fetch_messages())
...
def create_client(config: ProviderConfig) -> LLMClient:
    ...
    elif config.protocol == "openai-compatible":
...
async def resolve_context_window(config: ProviderConfig) -> None:
    """
    context window 解析的：对于 anthropic 协议的 provider，
    从 {base_url}/v1/models/{model} 自动拉取一次模型的 max_input_tokens，
    并通过 set_fetched_context_window 缓存到 ``config``
    """
