...
def clone_registry_for_fork(parent_registry: ToolRegistry) -> ToolRegistry:
    """
    Fork 专用：复制父注册表的全部工具，不做任何过滤。遇到 AgentTool 实例时浅复制并标记 query_source，
    确保 fork 子 Agent 不能再次 fork（运行时拦截），同时保持工具定义与父 Agent 字节一致以命中 prompt cache。
    """
    import copy
