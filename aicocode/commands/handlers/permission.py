...
async def handle_permission(ctx: CommandContext) -> None:
    ...
    if sub == "":
        ...
        checker = ctx.agent.permission_validator
        rule_count = 0
        ...
        if not rule_str:
            ...
            return
        from aicocode.Permissions.rules import Rule, parse_rule
        ...
        if checker and checker.rule_engine:
            ...
            ctx.ui.add_system_message(f"规则已添加: {rule.tool_name}({rule.pattern}) → {rule.permission}")
        else:
            ctx.ui.add_system_message("规则引擎未初始化")
