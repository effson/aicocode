"""
从 .aicocode/commands/ 目录加载自定义 Markdown 命令。
...
---
description: 简短描述这个命令是做什么的（可选）
argument-hint: 提示用户输入什么参数（可选）
aliases:
  - 别名1
  - 别名2
---
这里是发送给 AI 的提示词正文。
可在这里使用 $ARGUMENTS 代表用户输入参数。
"""
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.aicocode/commands/
      2. <work_dir>/.aicocode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".aicocode" / "commands"))
    dirs.append(str(Path(work_dir) / ".aicocode" / "commands"))
